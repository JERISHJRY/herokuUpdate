...
const TableProductContainer = (props) => {
  ...
  const subTotal = () => {
    let subtotal = 0;
    subtotal = cartProductItems.map(
      (item) =>
        item.count *
        (item.discount ? offer(item.price, item.discount) : item.price)
    );
  ...
  const deleteHandler = (item, id) => {
    cartProductItems.filter((item) => {
      return item.web_id === id ? (item.incart = false) : (item.incart = true);
    });
                ...
                <tr key={index}>
                  ...
                  <td className="cart_quantity">
                    <div className="cart_quantity_button">
                      <button
                        ...
                        onClick={(event) => addCount(event, cartitem.web_id)}
                      >
                        +
                      </button>
                      <input
                      ...
                      <button
                        ...
                        onClick={(event) => deleteCount(event, cartitem.web_id)}
                      >
                        {" "}
                        -
                      </button>
                  ...
                  <td className="cart_delete">
                    <button
                      ...
                      href=""
                      onClick={() => deleteHandler(cartitem, cartitem.web_id)}
                    >
