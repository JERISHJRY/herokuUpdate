...
const Apiwrapper=(props)=>{
    ...
    useEffect(() => {
        if(location.search == ""){
           dispatch(Server("get","/product/items",null,actionType.PRODUCT_GET));      
        }
        else{
            ...
            if(key == "price"){
                ...
                dispatch(Server("get",`/product/priceFilter/:${price}`,null,actionType.PRODUCT_GET));
            }
            else if(key=="image_name"){
                let searchValue=payload[key];
                dispatch(Server("get",`/product/search/:${searchValue}`,null,actionType.PRODUCT_GET));
            }
            else{
                dispatch(Server("post","/product/filterProduct",payload,actionType.PRODUCT_GET));
            }
        }
    },[dispatch,location]);
